...
export function ImpulseDetailTemplate({
  ...
  const calculateConfidence = () => {
    ...
    const confidenceLevel: 'high' | 'mid' | 'low' | undefined = tierProgress >= 0.75
      ? undefined
      : tierProgress >= 0.5
      ? 'high'
      : tierProgress >= 0.25
  ...
  const buildConfidenceClasses = (): string => {
    ...
    return !confidenceLevel ? confidenceStyles.background : confidenceStyles.pulse[confidenceLevel];
  };
